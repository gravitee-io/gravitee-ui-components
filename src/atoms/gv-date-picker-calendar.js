 ...
 */
class GvDatePickerCalendar extends LitElement {
  ...
  _renderCurrentNav () {
    return html`
      <div class="current">
         ${this.yearMode ? '' : html`<gv-button link @click="${this._onSelectMonth}">${until(this.computeName(this.month, this.year))}</gv-button>`}
        <gv-button link @click="${this._onSelectYear}"><span class="monthName">${this.computeCurrentYearName()}</span></gv-button>
      </div>
    `;
  }
  ...
  render () {
   ...
   ${this.time && !this.monthMode && !this.yearMode ? html`
    <div class="foot foot-time"><gv-button primary 
    @click=${this._onDateTimeSelected} 
    .disabled="${this.disableValidation === true}">ok</gv-button></div>` : ''}
`;
  }

  isMinutesDisabled (value) {
    if (this.dateFrom) {
      let selectedDate = new Date(this.dateFrom * 1000);
      if (this.min) {
        const minDate = new Date(this.min * 1000);
        selectedDate = setHours(selectedDate, this._hour);
        selectedDate = setMinutes(selectedDate, value);
  ...
  isHoursDisabled (value) {
    if (this.dateFrom) {
      ...
      if (this.min) {
        const minDate = new Date(this.min * 1000);
        selectedDate = setHours(selectedDate, value);
        if (isBefore(selectedDate, minDate)) {
          return true;
        }
      }
  ...
  _updateTime () {
    ...
    if (this.isHoursDisabled(this._hour) || this.isMinutesDisabled(this._minute)) {
      this._hour = null;
      this._minute = null;
    }
