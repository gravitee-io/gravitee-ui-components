...
export class GvSchemaFormControl extends LitElement {
  ...
  _renderControl () {
    ...
    if (this.value == null && this.control.default != null) {
      ...
      dispatchCustomEvent(this, 'init', { value: this.value, currentTarget: element, control: this.control });
    }
    if (this.value != null) {
      if (this.control.type === 'boolean' || this.control.type === 'array') {
        element.value = this.value;
      }
      else {
        element.value = this.value.toString ? this.value.toString() : this.value;
      }
    }
  ...
  _onInput (e) {
    ...
    const value = e.detail && e.detail.value ? e.detail.value : e.detail;
