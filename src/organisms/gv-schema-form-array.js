...
export class GvSchemaFormArray extends LitElement {
  ...
  _renderValue (value, index) {
    ...
    return html`<div class="form__item" @mouseleave="${this._onMouseLeave}">
      <div class="form__item-label">
        <label>${this.schema.title}</label><gv-button link small @gv-button:click="${this._onRemove.bind(this, index)}" icon="general:close" title="Remove"></gv-button>
      </div>
      <gv-schema-form-control .id="${id}" .control="${control}" .value="${value}" ?required="${isRequired}" ?disabled="${isDisabled}"></gv-schema-form-control>
    </div>
    `;
  }
  ...
  render () {
    ...
    return html`<div class="form__control-array">
                  ...
                  <div class="form__item-group-header">
